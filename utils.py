...
def prepare_training_data(items_data, return_y=True):
    ...
    for item in items_data:
        X.append([
            ...
            item["Buy Limit"],
            item["ROI"],
            item["High Volume"],  # Add high volume feature
            item["Low Volume"],  # Add low volume feature
            item["Price Fluctuation"]  # Add price fluctuation feature
        ])
        y.append(item["Potential Profit (After Tax)"])

    X = np.array(X)
    scaler = StandardScaler()  # Create a StandardScaler object
    X = scaler.fit_transform(X)  # Scale the features using StandardScaler
